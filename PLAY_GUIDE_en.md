# TRE A2-Release Play Guide

This guide is intended for players who want to directly try the current Web demo. Follow this document in order to select or import a Story Pack, create a session, submit turns, inspect scenes and memory, and continue playing after restart. The current recommendation is `/app`, while `/play` is retained as a legacy comparison entry.
...
On Windows, the recommended startup path is the helper script:
...
- Right state area: character status, inventory/equipment, quests, Story Pack management, memory summary, media, and sandbox control buttons. Character info is driven by backend session data and shows `--` before a session is created.
...
2. Select `echoes_under_red_lantern` in the Story Pack selector, or provide background text if no pack is selected.
3. Click `New Session`.
4. Wait for the GM opening narrative to appear in the turn log.
5. Read the current scene, focusing on location description, exits, visible objects, and suggested actions.
6. Click a suggested action on the page, or enter `观察周围` in the input box and click `Send`.
7. Continue entering 3 to 5 clear actions, for example:
...
8. Click the memory panel's `Read` button to view recent memory text (generated by valid turns and segment summary).
9. Click the memory panel's `Refresh` button to trigger backend re-computation of the summary and confirm memory text updates.
10. After closing and restarting `uv run python app.py`, enter the original `session_id` in the top session input box and click `Load` to continue playing.

## 5. Story Pack flow

The current local demo pack is `story_packs/echoes_under_red_lantern`, titled "赤灯下的回声". Validate it from the command line:

```bash
uv run python -m tools.packs.validate story_packs/echoes_under_red_lantern
```

`examples/story_packs/` still contains `demo_a2_core` and an external Red Lantern copy for examples, import demos, and regression tests.

The right-side Story Pack management panel in `/app` provides three release-stage actions:

- `Preview`: read the selected pack's manifest, summary, and scene preview.
- `Import`: submit JSON draft content to `POST /api/story-packs`; the backend writes it to disk and validates it again.
- `Delete`: delete the local pack content directory without deleting historical sessions. Old sessions keep the pack metadata frozen at creation time.

Minimum import payload shape:
...
```json
{
  ...
  "scenes": {
    "start": {
      "scene_id": "start",
      "display_name": "Entry Scene",
      "summary": "The player enters a playable custom starting point.",
      "exits": [],
      "interactables": [],
      "visible_npcs": [],
      "visible_items": []
  ...
  "lore": {
    "world.md": "# Custom Release Pack\n\nWorld background goes here."
  }
}
```

## 6. How to enter actions
...
## 7. Scene, memory, and suggested actions
...
## 11. How to read character status
...
On Windows, pytest temporary-directory cleanup or Vitest/esbuild startup can hit permission noise. If assertions did not fail, rerun with a repository-local `--basetemp test_runs/<name>` or repeat the same command outside the sandbox.
...
Local model startup may be slow on first use. Confirm Ollama is running and that `qwen3.5:9b` and `bge-m3` are available.
