...
本指南面向想直接试玩当前 Web Demo 的玩家。按本文顺序操作，可以完成一次新会话创建、回合输入、场景查看、记忆查看、沙盒并入/回滚和重启后继续游玩。
...
7. 点击 `历史` 查看已发生的回合。
8. 点击 `记忆` 查看近期剧情摘要。
9. 需要保存当前沙盒变化时点击 `并入主线`；需要放弃当前沙盒变化时点击 `回滚沙盒`。
10. 关闭并重新启动 `python app.py` 后，可把顶部“会话”输入框填回原 `session_id`，点击 `加载` 继续游玩。
...
- `并入主线`：把当前沙盒剧情变化保存到主线状态。
- `回滚沙盒`：放弃当前沙盒剧情变化，恢复到主线状态。

如果只是普通试玩，不确定该选哪个，可以先继续正常输入行动；需要保留当前分支时再并入，需要放弃当前分支时再回滚。
...
如果只想验证确定性链路，可在 `config/agent_model_config.yml` 中关闭 `bindings.agents.gm.enabled`，重启服务后会走模板叙事。
