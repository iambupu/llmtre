...
def _build_sandbox_response_payload(
    ...
    入参：session_id（str）：会话 ID；request_id（str）：幂等请求号；scope（str）：幂等作用域；
        flag_key（str）：成功标记字段；payload（dict[str, Any]）：run_turn 输出；
        persisted_turn_id（int）：落盘回合号；trace（dict[str, Any]）：可回传 trace 结构。
...
def _persist_sandbox_action_result(
    context: Any,
    session_id: str,
    request_id: str,
    action_text: str,
    ...
    入参：context（Any）：API 运行时上下文；
        session_id/request_id/action_text/scope/flag_key：路由参数；
        fresh_session（dict[str, Any]）：锁内最新会话；payload（dict[str, Any]）：run_turn 输出。
...
def _sandbox_action(
    ...
    with session_lock:
        ...
        try:
            ...
            trace_id, trace = _build_sandbox_post_run_error(payload, stage, err, scope)
            logger.exception(
