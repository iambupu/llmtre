...
4. 点击 `历史`，确认回合历史可以读取。
5. 点击 `记忆` 和 `刷新记忆`，确认近期记忆文本（拼接/分段摘要）可以读取和更新。
6. 输入模糊行动，例如 `过去`，确认系统返回澄清问题而不是崩溃。
7. 在调试面板 `配置` 中切换 `流式输出` / `普通输出`，确认两种提交方式都可用。
8. 沙盒按钮按 A1 实验功能验收，不作为普通试玩按钮测试：
   - 先确认当前测试会话已经明确进入沙盒分支，且 Shadow 状态中存在本次分支产生的状态变化。
   - 再测试 `并入主线`，确认接口成功后 Active 状态包含预期变更，且 Active 主线状态未被清空。
   - 重新创建或进入一个独立沙盒分支后，再测试 `回滚沙盒`，确认 Shadow 变化被丢弃且 Active 状态保持原样。
   - 禁止只在普通 `sandbox_mode=false` 新会话中直接点击两个按钮并把“返回成功”视为验收通过。
